...
export function ReviewsCard() {
  ...
  const StarRating = ({ rating, label, isOverall = false, customIcon }: { rating: number; label: string; isOverall?: boolean; customIcon?: React.ReactNode }) => {
              ...
              [...Array(5)].map((_, i) => (
                <div
                  ...
                  className={`${isOverall ? 'w-6 h-6' : 'w-4 h-4'} ${
                    i < filledStars ? 'opacity-100' : (i === filledStars && partialStar ? 'opacity-100' : 'opacity-30')
                  }`}
                >
                  {customIcon}
                </div>
